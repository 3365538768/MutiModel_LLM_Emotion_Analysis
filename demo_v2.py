...
def save_tmp_img(visual_img):
    ...
    file_path = "E:/New_project/MiniGPT-4/temp_save" + file_name
...
def get_image_message(image_path, question):
    chat_state = CONV_VISION.copy()
    img_list = []
    # 只在第一次上传图片
    if len(img_list) == 0:
...
if __name__ == "__main__":
    input_root = "E:/New_project/MiniGPT-4/extract_data/CAER_validation_image"
    output_root = "E:/New_project/MiniGPT-4/extract_data/CAER_validation_image_text"
    os.makedirs(output_root, exist_ok=True)

    question = "what is the person's facial expression and emotion? describe as detailed as possible"
    ...
    for emotion_folder in os.listdir(input_root):
        ...
        for filename in os.listdir(input_emotion_path):
            ...
            output_txt_path = os.path.join(output_emotion_path, filename.replace(".jpg", ".txt"))

            print(f"Processing {image_path}...")

            try:
                message = get_image_message(image_path, question)
            ...
            torch.cuda.empty_cache()  # 清理cuda缓存避免爆显存
